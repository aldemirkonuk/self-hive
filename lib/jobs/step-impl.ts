...
function agentSystemPrompt(agent: PlannedAgent, custom: Record<string, Specialist>, bundle?: ResourceBundle): string {
...
export async function composeImpl(runId: string, problem: string, customAgents: Record<string, Specialist>, costByClass: Record<string, number>, bundle?: ResourceBundle) {
  ...
  const results = await Promise.all(layer.map(async (agent, idx) => {
    ...
    try {
      const stream = client.messages.stream({
        ...
        system: cachedSystem(agentSystemPrompt(agent, customAgents, bundle)),
...
// ─── STEP 6: finalize — persist scores, cost, allocate paper capital, close out ──
